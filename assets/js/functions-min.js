...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      // Add size variation based on mouse movement
      if (!isMoving) {
        isMoving = true;
        cursorLight.style.width = "750px";
        cursorLight.style.height = "750px";
      }
  
      // Reset size after mouse stops moving
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateActiveSlide(direction) {
        ...
        if (direction === 'next') {
            newIndex = (currentIndex + 1) % carouselItems.length;
        } else {
            newIndex = (currentIndex - 1 + carouselItems.length) % carouselItems.length;
        }
    ...
    prevButton.addEventListener('click', () => updateActiveSlide('prev'));
    nextButton.addEventListener('click', () => updateActiveSlide('next'));
});
...
document.querySelectorAll('.skill-card').forEach(card => {
    card.addEventListener('mouseover', () => {
        ...
        card.querySelector('.tooltip').style.visibility = 'visible';
    });

    card.addEventListener('mouseout', () => {
        card.querySelector('.tooltip').style.opacity = '0';
        card.querySelector('.tooltip').style.visibility = 'hidden';
    });
});
...
window.addEventListener('resize', function() {
    map.invalidateSize(); // Pastikan ukuran peta diperbarui saat jendela diubah ukurannya
});
 // Koordinat Gambir, Jakarta Pusat
 const lat = -6.173110;
 const lng = 106.829361;

 // Inisialisasi peta dengan tema gelap
 const map = L.map('map').setView([lat, lng], 15);

 // Tambahkan dark theme tile layer
 ...
 marker.bindPopup("Gambir<br>Jakarta Pusat").openPopup();

 // Tambahkan circle area
 ...
 map.on('zoom', function() {
     circle.setRadius(500 / map.getZoom() * 10);
 });

 // Tambahkan interaksi hover pada marker
